...
class ResNet9:
    def __init__(self, num_classes):
        ...
        # self.block9 = ConvBlock(inchannels=512*28*28, outchannels=num_classes, kernel_size=1, stride=1, padding=0)

        self.block1 = ConvBlock(inchannels=1, outchannels=8)
        self.block2 = ConvBlock(inchannels=8, outchannels=16)
        ...
        self.block9 = ConvBlock(inchannels=64*28*28, outchannels=num_classes, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, X):
        ...
        self.out5 = relu(self.block5(self.out34))
    ...
    def backward(self, X, y_true, y_pred, learning_rate):
        ...
        output_gradient = self.block9.backward(output_gradient, learning_rate)

        output_gradient = output_gradient.reshape(self.out78.shape)
        output_gradient = relu_backward(output_gradient, self.out78)
        output_gradient = self.block78.backward(output_gradient, learning_rate)

        output_gradient = relu_backward(output_gradient, self.out6)
        output_gradient = self.block6.backward(output_gradient, learning_rate)

        output_gradient = relu_backward(output_gradient, self.out5)
        output_gradient = self.block5.backward(output_gradient, learning_rate)

        output_gradient = relu_backward(output_gradient, self.out34)
        output_gradient = self.block34.backward(output_gradient, learning_rate)

        output_gradient = relu_backward(output_gradient, self.out2)
        output_gradient = self.block2.backward(output_gradient, learning_rate)

        output_gradient = relu_backward(output_gradient, self.out1)
        output_gradient = self.block1.backward(output_gradient, learning_rate)
...
for epoch in range(epochs):
    ...
    for i in tqdm(range(0, x_train.shape[0], batch_size), leave=False, desc='train', ncols=0):
        ...
        # 反向传播并更新权重
        nn.backward(x_batch.reshape(x_batch.shape[0], 1,28,28), y_batch, y_pred, learning_rate * (0.5 ** (epoch // 20)) )
